...
const experiences = [
  {
    title:   'Research Assistant (MSc Project)',
    ...
    color:   'teal',
    bullets: [
      'RISE: Hardware Roots-of-Trust Reconsidered — developing reconfigurable hardware security primitives for RISC-V',
  ...
  },
  {
    ...
    company: 'Coacharya Foundation / PES University',
  ...
  return (
    ...
    >
      <div className="relative flex flex-col md:flex-row gap-0 md:gap-8 items-start">
        {/* Left spacer (desktop alternating) */}
        {side === 'right' && <div className="hidden md:block md:w-1/2" />}

        {/* Timeline dot (desktop) */}
        <div
          className={`hidden md:flex absolute left-1/2 -translate-x-1/2 w-5 h-5 rounded-full ${dotColor[exp.color]} shadow-lg mt-6 z-10 items-center justify-center`}
        >
        ...
        <div className={`w-full md:w-1/2 ${side === 'left' ? 'md:pr-12' : 'md:pl-12'}`}>
          <div className="bg-[#0b2d45]/50 border border-white/5 rounded-2xl p-6 hover:border-teal-400/40 hover:bg-[#0b2d45]/80 transition-all duration-400 group">
            {/* Header */}
            <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
              <div>
                <h3 className="text-white font-bold text-base leading-snug group-hover:text-teal-300 transition-colors duration-300">
                  {exp.title}
                </h3>
                <p className="text-teal-400 text-sm font-medium mt-0.5">{exp.company}</p>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold flex-shrink-0 ${typeStyle[exp.type]}`}>
                {exp.type}
              </span>
            </div>
    ...
    <section id="experience" className="py-28" style={{ background: 'linear-gradient(180deg,#0b1a2e 0%,#081424 100%)' }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          {/* Vertical spine */}
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-teal-400/60 via-teal-400/20 to-transparent -translate-x-px" />
