...
const roles = ['Software Developer', 'Cybersecurity Researcher', 'AI / ML Engineer', 'Hardware Security Specialist']
...
function useTypewriter(words) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setText(deleting ? word.slice(0, text.length - 1) : word.slice(0, text.length + 1)), delay)
...
function useGlitch() {
  ...
  useEffect(() => {
    const schedule = () => {
      ...
      return setTimeout(() => { setActive(true); setTimeout(() => { setActive(false); schedule() }, 600) }, wait)
    }
    const t = schedule()
    return () => clearTimeout(t)
  }, [])
...
function Particles() {
  const pts = useMemo(() => Array.from({ length: 28 }, (_, i) => ({
    ...
    opacity: (Math.random() * 0.22 + 0.06).toFixed(2),
  })), [])
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {pts.map(p => (
        <div key={p.id} className="absolute rounded-full bg-teal-400"
          style={{ left: `${p.x}%`, top: `${p.y}%`, width: p.size, height: p.size, opacity: p.opacity, animation: `float ${p.dur}s ease-in-out ${p.delay}s infinite` }} />
      ))}
    </div>
  )
}

function TextContent({ typed, glitch }) {
  const mag1 = useMagnetic(0.4)
  const mag2 = useMagnetic(0.4)
  const mag3 = useMagnetic(0.4)
  ...
  return (
    <div className="max-w-xl">
      ...
      <h1 className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-extrabold text-white leading-[1.0] mb-4 tracking-tight">
        <span className={glitch ? 'glitch-text' : ''} data-text="Mannpreeth">Mannpreeth</span>
        <br />
        <span className={`text-transparent bg-clip-text ${glitch ? 'glitch-text' : ''}`}
          data-text="MK"
      ...
      <p className="text-gray-400 text-sm sm:text-base leading-relaxed mb-8 max-w-md">
        MSc Cybersecurity &amp; AI student at the University of Sheffield, researching hardware security
        primitives. Experienced in AI/ML, full-stack development, and industrial automation.
      </p>

      <div className="flex flex-col sm:flex-row gap-4">
        {/* Magnetic button 1 */}
        <div ref={mag1.ref} onMouseMove={mag1.onMouseMove} onMouseLeave={mag1.onMouseLeave} style={mag1.style} className="inline-block">
          <a href="#projects"
            className="inline-flex items-center justify-center gap-2 bg-teal-400 text-[#07111f] px-8 py-3.5 rounded-full font-bold text-sm hover:bg-teal-300 transition-colors duration-200 shadow-lg shadow-teal-400/30">
            See My Work
        ...
        {/* Magnetic button 2 */}
        <div ref={mag2.ref} onMouseMove={mag2.onMouseMove} onMouseLeave={mag2.onMouseLeave} style={mag2.style} className="inline-block">
          <a href="#contact"
            className="inline-flex items-center justify-center gap-2 border border-white/25 text-white px-8 py-3.5 rounded-full font-semibold text-sm hover:border-teal-400 hover:text-teal-400 transition-colors duration-200">
            Contact Me
          </a>
        </div>

        {/* Magnetic button 3 */}
        <div ref={mag3.ref} onMouseMove={mag3.onMouseMove} onMouseLeave={mag3.onMouseLeave} style={mag3.style} className="inline-block">
          <a href="/Mannpreeth_MK_ UK Resume.pdf" download="Mannpreeth_MK_Resume.pdf"
            className="inline-flex items-center justify-center gap-2 border border-teal-400/40 text-teal-400 px-8 py-3.5 rounded-full font-semibold text-sm hover:bg-teal-400/10 transition-colors duration-200">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <section id="home" className="overflow-hidden" style={{ background: '#07111f' }}>
      ...
      <div className="md:hidden flex flex-col min-h-screen">
        ...
        <Particles />
        <div className="flex-1 px-6 pb-16 pt-4"><TextContent typed={typed} glitch={glitch} /></div>
      </div>
      ...
      <div className="hidden md:flex relative min-h-screen items-center">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-8 w-full pt-28 pb-20">
          <TextContent typed={typed} glitch={glitch} />
        </div>
