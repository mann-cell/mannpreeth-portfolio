...
const projects = [
  {
    ...
    description:
      'Real-time crowd monitoring using YOLO object detection. Interactive React.js dashboard visualises live density analytics with smart recommendations.',
    tags: ['Flask', 'YOLO', 'TensorFlow', 'PyTorch', 'React.js', 'AWS', 'Docker'],
    category: 'AI / ML',
    categoryColor: 'purple',
    github: 'https://github.com/mann-cell',
    live: null,
    icon: (
...
const catColors = {
  purple: { bg: 'bg-purple-500/15', text: 'text-purple-300', border: 'border-purple-400/30', icon: 'bg-purple-400/15 text-purple-300' },
  blue:   { bg: 'bg-blue-500/15',   text: 'text-blue-300',   border: 'border-blue-400/30',   icon: 'bg-blue-400/15 text-blue-300'   },
  red:    { bg: 'bg-red-500/15',    text: 'text-red-300',    border: 'border-red-400/30',    icon: 'bg-red-400/15 text-red-300'    },
}
...
function ProjectCard({ project, delay }) {
  ...
  return (
    <div ref={ref} className={`fade-up ${inView ? 'in-view' : ''}`} style={{ transitionDelay: `${delay}ms` }}>
      <div className="h-full bg-[#0b2d45]/40 border border-white/5 rounded-2xl p-6 flex flex-col hover:border-teal-400/40 hover:-translate-y-2 hover:shadow-2xl hover:shadow-teal-900/30 transition-all duration-300 group">
        {/* Top row */}
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {project.tags.map(tag => (
            <span key={tag} className="text-xs bg-white/5 border border-white/10 text-gray-300 px-2.5 py-1 rounded-full font-medium">
              {tag}
            </span>
          ))}
        </div>
